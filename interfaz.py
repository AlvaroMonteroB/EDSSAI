...
class TestHirschbergApp:
    ...
    def show_camera_screen(self, cell_id):
        ...
        self.captured_frame = None # Resetear frame capturado al entrar
        ...
        # Usar tamaño definido para el label
        ...
        self.take_photo_button = tk.Button(button_frame, text="Tomar Foto", command=self.take_photo, state=tk.DISABLED) # Deshabilitado hasta que la cámara inicie
        ...
        back_button = tk.Button(self.root, text="Regresar", command=self.show_grid_screen) # Este botón sí debe liberar cámara
        back_button.pack(pady=5)

        # Lógica de mostrar imagen guardada o iniciar cámara
        if cell_id in self.photos:
             self.show_saved_image() # Esta función maneja estados de botón
        else:
            self.start_camera() # Iniciar cámara (habilita botón "Tomar Foto" si tiene éxito)
    ...
    def take_photo(self):
        """
        Captura una foto. PARA PRUEBAS: Carga imagen de Aron/ en lugar de usar cámara.
        """
        print("Presionado 'Tomar Foto'")
        # 1. (MODO PRUEBA) Cargar imagen simulada en lugar de leer de cámara
        # --------------------------------------------------------------
        # Definir la ruta a la imagen de prueba
        ruta_aron = f'Aron/{self.current_cell_id}.jpg' # Asegúrate que la extensión sea correcta
        print(f"--- MODO PRUEBA ACTIVO: Intentando cargar desde {ruta_aron} ---")

        # Leer la imagen de prueba desde el archivo
        frame_simulado = cv2.imread(ruta_aron) # Cargar en color BGR por defecto

        # Verificar si la carga fue exitosa
        if frame_simulado is None:
            messagebox.showerror("Error Modo Prueba", f"No se pudo cargar la imagen de prueba:\n{ruta_aron}\n\nVerifica que el archivo exista en la carpeta 'Aron'.")
            # ¿Qué hacer si falla? Podríamos simplemente no hacer nada más.
            # O podrías intentar reiniciar la cámara con retake_photo()
            return # Salir de la función si no se cargó la imagen de prueba

        # Si la carga fue exitosa, este 'frame_simulado' se usará como si fuera de la cámara.
        print("--- MODO PRUEBA: Imagen cargada exitosamente. ---")
        self.captured_frame = frame_simulado # GUARDAR EL FRAME SIMULADO

        # 2. Detener la cámara (incluso en modo prueba, para liberar el recurso)
        # --------------------------------------------------------------
        self.release_camera_if_active() # Usar helper

        # 3. Mostrar la imagen "capturada" (la simulada) en el label
        # --------------------------------------------------------------
        # Usamos la función de conversión/display con el frame SIMULADO
        self.photo_display = self.convert_frame_for_display(self.captured_frame)
        if self.photo_display:
            self.camera_label.config(image=self.photo_display, width=DISPLAY_WIDTH, height=DISPLAY_HEIGHT)
            self.camera_label.image = self.photo_display # Guardar referencia
        else:
            # Si la conversión falla, mostrar error o placeholder
            self.camera_label.config(text="Error al mostrar\nimagen de prueba", image=None)
            messagebox.showerror("Error", "Hubo un problema al preparar la imagen de prueba para visualización.")
            # Aún así, podríamos permitir aceptar si self.captured_frame es válido
            # Pero por seguridad, mejor deshabilitar si no se ve.
            self.accept_photo_button.config(state=tk.DISABLED)
            self.take_photo_button.config(text="Reintentar Carga", command=self.take_photo) # Cambiar comando a sí mismo
            return # Salir si no se pudo mostrar

        # 4. Actualizar estado de botones
        # --------------------------------------------------------------
        self.take_photo_button.config(text="Volver a Tomar", command=self.retake_photo) # Retake SÍ reinicia cámara real
        self.accept_photo_button.config(state=tk.NORMAL) # Habilitar Aceptar
